...
def details_lvl1_before_update(session, flush_context, instances):
    ...
    for instance in session.deleted:
        if isinstance(instance, ads_solution_details): #? CHANGE FOR EACH OBJECT
            # update `ads_solution_data`, attribute `_useless_to_keep_history`
            if type(sa.orm.attributes.get_history(instance, 'ads_solution_data').unchanged[0]) == type(list()):  #? pay ATTN to PARENT RELATIONSHIP NAME
                _taregt_to_update = sa.orm.attributes.get_history(instance, 'ads_solution_data').unchanged[0][0] #? pay ATTN to PARENT RELATIONSHIP NAME
            else:
